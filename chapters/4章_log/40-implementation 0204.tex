...
\begin{itemize}
    \item base\_dirディレクトリ:\\
          ...
          %   \begin{itemize}
          %       \item currentディレクトリ:\\
          %             \toolName の初回実行時に取得した、Webページの変更前画像と変更前HTMLコードを保持する。\\
          %             なお、開発者が「\$ make save」(\ref{subsec:MixVRT_evaluate}を参照)を実行することで、latestディレクトリ内のデータに置き換わる。
          %       \item initialディレクトリ:\\
          %             \toolName の初回実行時に取得した、Webページの変更前画像と変更前HTMLコードを保持する。\\
          %             データの更新はない。      
          %       \item latestディレクトリ:\\
          %             \toolName の2回目以降の実行時に取得した、Webページの変更後画像と変更後HTMLコードを保持する。\\
          %             実行ごとにデータを更新する。
          %   \end{itemize}
...
\begin{table}[tp]
    ...
    \begin{tabular}{c|l}
        ...
        \begin{tabular}{l}枠付き変更前HTMLコード\\\end{tabular}  & \begin{tabular}{l}影響箇所における削除または変更があった画面要素を\\赤枠で囲むCSSクラスを付加した、変更前HTMLコード\end{tabular}                                   \\ \hline
        \begin{tabular}{l} 枠付き変更後HTMLコード\\\end{tabular} & \begin{tabular}{l}影響箇所における追加または変更があった画面要素を\\緑枠で囲むCSSクラスを付加した、変更後HTMLコード\end{tabular}                                   \\ \hline
        枠付き変更前画像           & 赤枠付けを行った、Webページの変更前画像                      \\ \hline
        枠付き変更後画像           & 緑枠付けを行った、Webページの変更後画像                      \\ \hline
...
% 表示部は、データ管理部によってdisp\_dirディレクトリ内に保持されたデータを受け取り、
