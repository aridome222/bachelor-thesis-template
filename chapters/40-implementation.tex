...
なお、枠付き変更前HTMLコードは、影響箇所(\ref{cha:Function}を参照)における削除箇所に赤枠をつけるCSSクラス(節を参照)を付加した、変更前HTMLコードと定義する。
...
\begin{enumerate}
    ...
    \item 二値化した「差分箇所赤枠強調マスク画像」に対して、cv2.findContours関数を用いて、輪郭リストcontours\_img\_bfを取得する。
    \item 二値化した「影響箇所赤枠強調マスク画像」に対して、cv2.findContours関数を用いて、輪郭リストcontours\_html\_bfを取得する。
    \item contours\_img\_bfの各輪郭要素の数だけループして、以下の処理を行う。
          ...
          \end{enumerate}
    \item matchの値がTrueなら、contours\_img\_bfの輪郭要素を、差分箇所の赤枠と影響箇所の赤枠が一致しなかった場合の差分箇所の輪郭のみを要素に持つunique\_contours\_bfに格納する。
    \item 「差分箇所緑枠強調マスク画像」と「影響箇所緑枠強調マスク画像」に対しても、同様に1から7の操作を行う。
\end{enumerate}
...
\begin{enumerate}
    ...
    \item threshold関数(\ref{sec:opencv})で二値化を行う。
