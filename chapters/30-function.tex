...
\begin{enumerate}
    \item 「\$ make test URL="WebページのURL"」で事前準備を行う。
    \item Webページの変更を行う。
    \item 「\$ make test URL="WebページのURL"」で視覚的回帰テストを実行する。
    \item 「http://localhost:5000/MixVRT\_diff」にアクセスし、テスト結果を確認する。
    \item テスト結果を評価する
          \begin{itemize}
              \item 問題なし:「\$ make save」で比較用画像と比較用HTMLコードを更新する。
