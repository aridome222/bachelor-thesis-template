...
\toolName 実行時のコマンドライン引数によって、\toolName による視覚的回帰テストを行う対象のWebページのURLを指定する。
...
\toolName の2回目以降実行によって取得したWebページのテスト対象画像とテスト対象HTMLコードを、比較用画像と比較用HTMLコードに対して\toolName による視覚的回帰テストを行うことで、
以下に示すPNG形式の画像を生成する。
...
\end{itemize}
\toolName は、生成したPNG形式の画像を、Flask(\ref{sec:Flask}節を参照)を使用して構築したローカルサーバ上で動作するWebページに出力し、表示する。
開発者は、以下のローカルWebページのURLにアクセスすることで、\toolName が生成したPNG形式の画像を確認できる。
